...
// Read maxRounds from query on init so child component gets it immediately
...
const handleGoBack = async () => {
  ...
  try {
    // Try graceful simulation environment shutdown first
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment is not running, check whether process still needs to be stopped
...
const loadGraph = async (graphId) => {
  // During simulation, avoid fullscreen loading to prevent flicker
...
onMounted(() => {
  ...
  // Log maxRounds config (already read from query at init)
  if (maxRounds.value) {
