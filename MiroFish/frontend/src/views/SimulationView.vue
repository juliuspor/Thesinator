 ...
 * When a user returns from Step 3 to Step 2, assume they want to exit simulation
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Try graceful environment shutdown
      try {
        ...
        addLog(`Exception while closing environment: ${closeErr.message}`)
        // If graceful shutdown throws, force stop
        await forceStopSimulation()
      }
    } else {
    ...
    // Environment status check failure should not block next flow
...
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`Exception during force stop: ${err.message}`)
  }
}
