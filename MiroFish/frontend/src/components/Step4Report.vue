...
const parseInsightForge = (text) => {
  ...
  try {
    // 提取分析问题
    const queryMatch = text.match(/(?:Analysis Query|分析问题):\s*(.+?)(?:\n|$)/i)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // 提取预测场景
    const reqMatch = text.match(/(?:Forecast Scenario|预测场景):\s*(.+?)(?:\n|$)/i)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // 提取统计数据 - 匹配"相关预测事实: X条"格式
    const factMatch = text.match(/(?:Relevant Forecast Facts|相关预测事实):\s*(\d+)/i)
    const entityMatch = text.match(/(?:Involved Entities|涉及实体):\s*(\d+)/i)
    const relMatch = text.match(/(?:Relationship Chains?|关系链):\s*(\d+)/i)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // 提取子问题 - 完整提取，不限制数量
    const subQSection = text.match(/###\s*(?:Analyzed Sub-Queries|分析的子问题)\n([\s\S]*?)(?=\n###|$)/i)
    if (subQSection) {
    ...
    // 提取关键事实 - 完整提取，不限制数量
    const factsSection = text.match(/###\s*(?:\[Key Facts\]|【关键事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/i)
    if (factsSection) {
    ...
    const entitySection = text.match(/###\s*(?:\[Core Entities\]|【核心实体】)\n([\s\S]*?)(?=\n###|$)/i)
    if (entitySection) {
    ...
    // 提取关系链 - 完整提取，不限制数量
    const relSection = text.match(/###\s*(?:\[Relationship Chains\]|【关系链】)\n([\s\S]*?)(?=\n###|$)/i)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // 提取当前有效事实 - 完整提取，不限制数量
    const activeSection = text.match(/###\s*(?:\[Current Active Facts\]|【当前有效事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/i)
    if (activeSection) {
    ...
    const histSection = text.match(/###\s*(?:\[Historical\/Expired Facts\]|【历史\/过期事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/i)
    if (histSection) {
    ...
    // 提取涉及实体 - 完整提取，不限制数量
    const entitySection = text.match(/###\s*(?:\[Involved Entities\]|【涉及实体】)\n([\s\S]*?)(?=\n###|$)/i)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*(?:Interview Coverage|采访人数):\*\*\s*(\d+)\s*\/\s*(\d+)/i)
    if (countMatch) {
    ...
    // 提取采访对象选择理由
    const reasonMatch = text.match(/###\s*(?:Interviewee Selection Rationale|采访对象选择理由)\n([\s\S]*?)(?=\n---\n|\n###\s*(?:Interview Transcripts|采访实录))/i)
    if (reasonMatch) {
    ...
    // 提取每个采访记录
    const interviewBlocks = text.split(/####\s*(?:采访|Interview)\s*#\d+:/i).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          if (interview.redditAnswer && !isNoPlatformReply(interview.redditAnswer)) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && !isNoPlatformReply(interview.twitterAnswer)) {
    ...
    // 提取采访摘要
    const summaryMatch = text.match(/###\s*(?:Interview Summary & Core Insights|采访摘要与核心观点)\n([\s\S]*?)$/i)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // 提取结果数量
    const countMatch = text.match(/(?:Found|找到)\s*(\d+)\s*(?:relevant items|条)?/i)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // 提取相关事实 - 完整提取，不限制数量
    const factsSection = text.match(/###\s*(?:Relevant Facts|相关事实):\n([\s\S]*)$/i)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
    ...
    // 尝试提取边信息（如果有）
    const edgesSection = text.match(/###\s*(?:Related Edges|相关边):\n([\s\S]*?)(?=\n###|$)/i)
    if (edgesSection) {
    ...
    // 尝试提取节点信息（如果有）
    const nodesSection = text.match(/###\s*(?:Related Nodes|相关节点):\n([\s\S]*?)(?=\n###|$)/i)
    if (nodesSection) {
