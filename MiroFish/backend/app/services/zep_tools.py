...
class EdgeInfo:
    ...
    def to_text(self, include_temporal: bool = False) -> str:
        ...
        if include_temporal:
            ...
            base_text += f"\nTime Window: {valid_at} - {invalid_at}"
            if self.expired_at:
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        if self.semantic_facts:
            text_parts.append(f"\n### [Key Facts] (quote these source facts in the report)")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### [Current Active Facts] (verbatim simulation outputs)")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # 过滤包含问题编号的垃圾内容（问题1-9）
                skip = bool(re.search(r'(问题\d+|Question\s*\d+)', clean_quote, flags=re.IGNORECASE))
                if skip:
                    continue
                # 截断过长内容（按句号截断，而非硬截断）
                if len(clean_quote) > 150:
                    zh_dot_pos = clean_quote.find('\u3002', 80)
                    en_dot_pos = clean_quote.find('.', 80)
                    dot_candidates = [pos for pos in [zh_dot_pos, en_dot_pos] if pos > 0]
                    dot_pos = min(dot_candidates) if dot_candidates else -1
                    if dot_pos > 0:
...
class ZepToolsService:
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            node = self._call_with_retry(
                ...
                operation_name=f"fetch_node_detail(uuid={node_uuid[:8]}...)"
            )
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are an expert question analyst. Break a complex question into multiple independently observable sub-questions in the simulation world.

Requirements:
1. Each sub-question must be specific enough to map to agent behavior or events in the simulation.
...
Return a JSON list of sub-questions."""
        ...
        try:
            ...
            return [
                query,
                f"Key participants related to: {query}",
    ...
    def interview_agents(
        ...
        logger.info(f"InterviewAgents live API interview: {interview_requirement[:50]}...")
        ...
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Based on your persona, memory, and actions, "
            "answer the following questions directly in plain text.\n"
            "Response requirements:\n"
            "1. Use natural language only; do not call any tools\n"
            "2. Do not return JSON or tool-call formats\n"
            "3. Do not use Markdown headings (such as #, ##, ###)\n"
            "4. Answer each question by index, and start each answer with 'Question X:' (or '问题X:' for compatibility)\n"
            "5. Separate answers to different questions with a blank line\n"
            "6. Provide substantive content; each answer should be at least 2-3 sentences\n\n"
        )
        ...
        try:
            ...
            if not api_result.get("success", False):
                ...
                result.summary = (
                    ...
                    "Please verify the OASIS simulation environment status."
                )
                return result
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                clean_text = re.sub(r'(?:问题\d+[：:]|Question\s*\d+[：:])\s*', '', clean_text, flags=re.IGNORECASE)
                clean_text = re.sub(r'【[^】]+】', '', clean_text)

                # 策略1（主）: 提取完整的有实质内容的句子（中英文标点）
            ...
            result.summary = f"Interview failed: {str(e)}. Simulation environment may be stopped. Ensure OASIS is running."
            return result
        except Exception as e:
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are an expert interview planner. Select the most suitable interviewees from the simulated agent list based on the interview objective.

Selection criteria:
1. Agent identity/profession is relevant to the interview topic.
2. Agent is likely to provide unique or high-value perspectives.
3. Ensure perspective diversity (supporting/opposing/neutral/professional, etc.).
4. Prioritize roles directly related to the event.

Return JSON format:
{
    "selected_indices": [selected agent index list],
...
Select up to {max_agents} agents who are best suited for interview, and explain your rationale."""
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional interviewer. Generate 3-5 in-depth interview questions based on the interview requirement.

Question requirements:
1. Open-ended questions that invite detailed answers.
2. Questions where different roles may respond differently.
3. Cover multiple dimensions: facts, views, feelings.
4. Natural language, like a real interview.
5. Keep each question concise and clear (about one sentence).
6. Ask directly; do not prepend background exposition.
...
Generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = """You are a professional news editor. Generate an interview summary from multiple interviewee responses.

Summary requirements:
1. Distill key viewpoints across interviewees.
2. Highlight consensus and disagreement.
3. Surface valuable quotes.
4. Stay objective and neutral.
5. Keep it concise (roughly within 1000 words).
