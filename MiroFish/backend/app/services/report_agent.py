...
class ReportLogger:
    ...
    def log_planning_start(self):
        ...
        self.log(
            ...
            stage="planning",
            details={"message": "Started outline planning"},
        )
    ...
    def log_section_start(self, section_title: str, section_index: int):
        ...
        self.log(
            ...
            section_title=section_title,
            section_index=section_index,
            details={"message": f"Started section generation: {section_title}"},
        )
...
Deep insight retrieval: automatically decomposes questions and performs multi-dimensional graph retrieval (semantic search + entity analysis + relationship-chain tracing).
Returns: relevant source facts, core entity insights, and relationship-chain analysis."""

TOOL_DESC_PANORAMA_SEARCH = """\
Panorama search: retrieves a full view of the simulation and event evolution.
Returns: active facts, historical facts, and all involved entities."""

TOOL_DESC_QUICK_SEARCH = """\
Quick search: lightweight retrieval for simple lookups and fact checks.
Returns: facts most relevant to the query."""

TOOL_DESC_INTERVIEW_AGENTS = """\
Agent interviews: calls the live OASIS simulation and interviews running agents (not LLM-simulated stand-ins).
Automatically selects relevant agents and interviews across Twitter and Reddit for multi-role perspectives.
Returns: agent identity, dual-platform answers, key quotes, and perspective comparisons.
Requires the OASIS simulation environment to be running."""

# ── 大纲规划 prompt ──

PLAN_SYSTEM_PROMPT = """\
You are an expert author of future-forecast reports with a "god's-eye view" of the simulation world: you can observe every agent's behavior, statements, and interactions.

[Core Premise]
We built a simulation world and injected a specific "simulation requirement" as the key variable. The world evolution is a forecast of plausible futures. You are not reading mere "experiment data"; you are observing a "future rehearsal."

[Your Task]
Write a "Future Forecast Report" that answers:
1. Under our configured conditions, what happened in the future simulation?
2. How did different agent groups react and act?
3. What future trends and risks does this simulation reveal?

[Report Positioning]
- ✅ This is a simulation-based future forecast report answering "if this happens, what comes next?"
- ✅ Focus on projected outcomes: trajectory, group reactions, emergent effects, and potential risks.
- ✅ Agent behavior in the simulation is treated as a projection of future group behavior.
- ❌ Not an analysis of current real-world status.
- ❌ Not a generic opinion roundup.

[Section Count Limits]
- Minimum 2 sections, maximum 5 sections.
- No subsections required; each section should be complete and focused.
- Keep content concise and centered on core forecast findings.
- Design the section structure based on the forecast signal.
...
Note: `sections` must contain between 2 and 5 items."""
...
- Number of relationships produced: {total_edges}
- Entity type distribution: {entity_types}
- Number of active agents: {total_entities}

[Sample forecasted future facts]
{related_facts_json}

Review this future rehearsal from a "god's-eye view":
1. Under our configured conditions, what future state emerges?
2. How do different agent groups react and behave?
3. What notable future trends are revealed?

Design the most suitable report section structure based on these forecast outcomes.

[Reminder] section count must be 2-5, and content should stay concise and focused on core findings."""

# ── 章节生成 prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
You are writing a section of a "Future Forecast Report."

Report: {report_title} | Scenario: {simulation_requirement}
Current Section: {section_title}

[Core Rules]
1. You must call tools (2-3 times) to inspect the simulation world; all content must come from simulation data.
2. You must cite original agent statements in this format: > "quoted content..."
3. Keep output language consistent in English.
4. Faithfully present forecast results; do not fabricate details.

[Formatting Rules]
- ❌ Do not use Markdown headings (#, ##, ###, etc.).
- ✅ Use **bold text** for subsection labels when needed.
- ✅ Place each quote in its own paragraph with blank lines around it.

[Tools] (2-3 calls per section)
{tools_description}

[Workflow] Each reply must do exactly one of the following:
A. Tool call: after reasoning, output <tool_call>{{"name": "tool_name", "parameters": {{}}}}</tool_call>
B. Content output: output section content starting with "Final Answer:"

⚠️ Never include both a tool call and Final Answer in the same reply."""
...
[Important Reminders]
1. Read completed sections carefully and avoid repeating the same material.
2. Before writing, call tools to gather simulation evidence.
3. Mix different tools; do not rely on only one tool.
4. Report content must be grounded in retrieved results, not prior knowledge.
...
- ✅ Write body content directly; use **bold text** for subsection labels.

Start now:
1. Think about what information this section needs.
2. Call tools (Action) to gather simulation evidence.
3. After enough evidence is collected, output Final Answer (body only, no headings)."""
...
- If evidence is sufficient: output section content starting with "Final Answer:" (must cite evidence above).
- If more evidence is needed: call another tool.
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Notice] You have called tools {tool_calls_count} time(s), but at least {min_tool_calls} are required. "
    "Call tools again to gather more simulation data, then output Final Answer. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "You have only called tools {tool_calls_count} time(s); at least {min_tool_calls} are required. "
    "Please call a tool to fetch simulation evidence. {unused_hint}"
)

REACT_TOOL_LIMIT_MSG = (
    "Tool-call limit reached ({tool_calls_count}/{max_tool_calls}); you cannot call more tools. "
    'Immediately output section content based on current evidence, starting with "Final Answer:".'
)

REACT_UNUSED_TOOLS_HINT = (
    "\nTip: you have not used these tools yet: {unused_list}. Consider using diverse tools for multi-angle evidence."
)

REACT_FORCE_FINAL_MSG = 'Tool-call limit reached. Output "Final Answer:" now and produce the section content directly.'
...
[Rules]
1. Answer primarily based on the report content above.
2. Answer directly and avoid verbose internal monologue.
...
[Answer Style]
- concise and direct; avoid long essays
- cite key evidence using > quote format
...
class ReportAgent:
    ...
    # 最大工具调用次数（每个章节）- 优化：从5减少到3以降低token消耗
    MAX_TOOL_CALLS_PER_SECTION = 3

    # 最大反思轮数 - 优化：从3减少到2以降低token消耗
        ...
        return {
            "insight_forge": {
                ...
                "parameters": {
                    ...
                    "report_context": "Current report-section context (optional, helps produce better sub-queries)",
                },
            },
            "panorama_search": {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or objective (e.g., 'understand student views on a campus incident')",
    ...
    def plan_outline(
        ...
        try:
            ...
            return ReportOutline(
                ...
                sections=[
                    ReportSection(title="Forecast Scenarios and Core Findings"),
    ...
    def _generate_section_react(
        ...
        # ReACT循环 - 优化：减少迭代次数以降低token消耗
        tool_calls_count = 0
        max_iterations = 4  # 最大迭代轮数（优化：从5减少到4）
        min_tool_calls = 2  # 最少工具调用次数（优化：从3减少到2）
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                logger.warning(
                    ...
                    f"LLM returned both tool call and Final Answer (conflict #{conflict_retries})"
                )
                    ...
                    messages.append(
                        {
                            "role": "user",
                            "content": (
                                "[Format Error] Your single reply included both a tool call and Final Answer, which is not allowed.\n"
                                "Each reply must do exactly one of the following:\n"
                                "- Call one tool (output one <tool_call> block, without Final Answer)\n"
                                "- Output final content (start with 'Final Answer:' and do not include <tool_call>)\n"
            ...
            if has_final_answer:
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = (
                        f"(Unused tools you may try: {', '.join(unused_tools)})"
                        if unused_tools
                        else ""
                    )
                    messages.append(
                ...
                logger.info(
                    f"Section {section.title} complete (tool calls: {tool_calls_count})"
                )
            ...
            if has_tool_calls:
                ...
                if len(tool_calls) > 1:
                    logger.info(
                        f"LLM attempted {len(tool_calls)} tool calls; only executing the first: {call['name']}"
                    )
            ...
            # 直接将这段内容作为最终答案，不再空转
            logger.info(
                f"Section {section.title}: no 'Final Answer:' prefix detected; accepting raw LLM output (tool calls: {tool_calls_count})"
            )
        ...
        if response is None:
            ...
            final_answer = "Section generation failed: LLM returned an empty response. Please retry later."
    ...
    def generate_report(
        ...
        try:
            ...
            ReportManager.update_progress(
                ...
                "planning",
                15,
                f"Outline planning complete, {len(outline.sections)} sections in total",
                completed_sections=[],
            )
            ReportManager.save_report(report)
